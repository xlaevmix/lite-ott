...
async function getEpgProgress(chId) {
  ...
  let logo = channel?.icon || dummyIcon;
  ...
  return [current, next, percent, description, logo];
}
...
async function getCurrentEpgJson(chname) {
  if (epgApiUrl_now === "") return null;
  let currentepg = {};
  try {
    const url = parseApiUrl(epgApiUrl_now, { name: chname });
    const response = await getJson(url);

    const data = epgApiConverter[epgApi_provider](response);

    currentepg = {
      id: data.ch_id,
      names: [chname],
      icon: data.icon || dummyIcon,
      events: data.events || []
    };
...
function getChannelEpgJson(channelIDorName, dateYMD) {
  return new Promise((resolve, reject) => {
    if (epgApiUrl_day === "") reject(new Error("No epgApiUrl_day defined"));
    try {
      const url = parseApiUrl(epgApiUrl_day, { date: dateYMD, name: channelIDorName });
      getJson(url)
        .then(response => {
          const data = epgApiConverter[epgApi_provider](response);
          if (data && data.events) {
...
const epgApiConverter = {
  default(data) {
    return data;
  },
  definetv(data) {
    function mjdToUnix(mjdDateTime) {
      const [mjdStr, timeStr] = mjdDateTime.split(' ');
      const mjd = parseInt(mjdStr, 10);
      const [hours, minutes, seconds] = timeStr
        .split(':')
        .map(Number);

      // MJD epoch starts at 1858-11-17 00:00:00 UTC
      const mjdEpoch = Date.UTC(1858, 10, 17);
      const ms =
        mjdEpoch +
        mjd * 86400000 +
        hours * 3600000 +
    ...
    const result = {
      ...
      icon: data.icon.includes('epg.one')
        ? data.icon.replace('/img/', '/img2/')
        : data.icon,

      events: data.events.map(event => {
        const start = mjdToUnix(event.start_time);
        const stop = start + durationToSeconds(event.duration);
