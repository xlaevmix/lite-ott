...
/* 
epgApiUrl_now - url template for current/next event for channel
epgApiUrl_day - url template for whole day epg for channel
examples of url template: 
(
  epgApiUrl_now -   'http://example.com/epg/get_epg.php?name=${name}'
  epgApiUrl_day -   'http://example.com/epg/get_epg.php?date=${date}&name=${name}'
)
  Base URL of template 'http://example.com' can be replaced with 'window.location.origin' to avoid CORS issues when api server is on the same host as web player:
  (epgApiUrl_now = window.location.origin + '/${date}/${name}';)

  param "date": 'yyyymmdd' format.
  undefined parameters will be removed when parse url
  server response should be in format:
  start/stop - unix timestamps
  {
    "ch_id": "Channel 1",
    "ch_name": "Channel 1",
    "icon": "http://example.com/picons/ch1.png", //with UNESCAPED_SLASHES
    "events": [
        {
            "start": 1761044100,
            "stop": 1761047100,
...
//name of function for convert response format if not in standard format described above. function should return data in standard format.
const epgApi_provider = "default"; 
...
async function loadPlaylist() {
  ...
  if (!(playlist.length > 0)) {
    ...
    try {
      const text = await get_data(baseUrl, '*/*');
  ...
  if (playerType === "native") {
    if (video.canPlayType("application/vnd.apple.mpegurl")) {
      ...
      video.play().then(() => {
        ...
        console.error("Error (native player):", err.message);
        loadingstatus.textContent = "Error (native player): " + err.message;
        spinnerShow(true);
      });
    } else {
    ...
    if (Hls.isSupported()) {
      ...
      hls.once(Hls.Events.MANIFEST_PARSED, (event, data) => {
        video.play().then(() => {          
          ...
          console.error("Error (HLS player):", err.message);
          loadingstatus.textContent = "Error (HLS player): " + err.message;
          spinnerShow(true);
        });
      });
...
function setPlaylistTvgData(chname, id, isepg, logo) {
  const plEntry = playlist.find(ch => ch.name === chname);
      plEntry.tvg = plEntry.tvg || {};
      plEntry.tvg.id = id;
      plEntry.tvg.isepg = isepg;
      plEntry.tvg.logo = logo;
}
...
async function setChannelInfo(index) {
  ...
  const chId = epgSource === "xmltv" ? channel.tvg.id : channel.name;
  const isepg = channel.tvg.isepg;
  let currprog, nextprog, descr;
  currprog = nextprog = descr = "";
  let perc = 0;
  let logo = channel.tvg.logo || dummyIcon;
  if (chId !== "" && isepg !== "0") {    
    [currprog, nextprog, perc, descr, logo] = await getEpgProgress(chId);
    const last = 100 - perc;
    if (perc > 50) {
    ...
    if (isepg === "" && epgSource === "server") {      
      if (currprog !== "") {        
        setPlaylistTvgData(channel.name, chId, "1", logo);
        channels[index].querySelector("#logo").src = logo;
      } else {
        setPlaylistTvgData(channel.name, chId, "0", logo);
      }
    }
  }
...
async function setChannelInfoAll() {
...
async function infobarShow(show = true) {
  if (show) {
    ...
    if (currprog === "") {      
      ...
      if (isepg === "" && epgSource === "server") setPlaylistTvgData(channel.name, chId, "0", logo);
    } else {
      document.getElementById("infoprogress").style.display = 'inline-block';
      if (isepg === "" && epgSource === "server") setPlaylistTvgData(channel.name, chId, "1", logo);
    }    
  ...
  try {
    if (useDB) {
      ...
      for (const channel of allChannels) {
        if (!Array.isArray(channel.names)) continue;

        for (const name of channel.names) {
          if (typeof name === 'string') {
            channelMap.set(name.toLowerCase(), channel);
          }
        }
      }
      let i = 0;
      for (const plEntry of playlist) {
        i++;
        const percent = Math.floor((i / playlist.length) * 100);
        statusbar.textContent = `Processing channels ( ${percent}% )`;
        if (!plEntry?.name) continue;

        const match = channelMap.get(plEntry.name.toLowerCase());

        if (match) {
          plEntry.tvg = plEntry.tvg || {};
          plEntry.tvg.id = match.id;
          plEntry.tvg.isepg = "1";
          plEntry.tvg.logo = match.icon;
        }
      }
